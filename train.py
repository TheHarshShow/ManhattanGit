...
def train(band, x_patches_file,
    ...
    X_train, X_val, y_train, y_val, y_train_mask, y_val_mask = train_test_split(
        X_train_patches, y_train_patches, y_train_mask_patches, test_size=val_split, random_state=42)

    del X_train_patches, y_train_patches, y_train_mask_patches

    gc.collect()

    trainer_nir.fit([X_train],
                    [y_train, y_train_mask],
                    batch_size=32,
                    validation_data=([X_val], [y_val, y_val_mask]))
